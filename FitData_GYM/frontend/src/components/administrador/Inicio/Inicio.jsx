    ...
    <div className="flex flex-col items-center justify-center min-h-[80vh] text-center p-6 text-gray-100">
      ...
      <div className="grid md:grid-cols-2 gap-10 max-w-5xl w-full">
        {TARJETAS.map((tarjeta) => (
          <div
            key={tarjeta.titulo}
            className="bg-gray-800 p-8 rounded-xl shadow-xl border border-gray-700 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1"
          >
            <h2 className={`text-3xl font-bold mb-4 ${tarjeta.color}`}>{tarjeta.titulo}</h2>
            <p className="text-gray-300 leading-relaxed text-lg">{tarjeta.texto}</p>
          </div>
        ))}
      </div>
    </div>
