import useTarjetaProducto from './hooks/useTarjetaProducto';
import SelectorCantidad from './partes/SelectorCantidad';

const TarjetaProducto = ({ producto, onAgregar, onEditar, onEliminar }) => {
  const { cantidad, imgSrc, incrementar, decrementar, handleImageError } = useTarjetaProducto(producto);
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-2xl p-3 md:p-5 flex flex-col h-full shadow-xl relative transition-transform hover:-translate-y-1 duration-300 group overflow-hidden">
      <div className="absolute top-2 right-2 flex gap-1 z-10 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
        <button onClick={() => onEditar(producto)} className="bg-slate-900/90 backdrop-blur-md border border-slate-600 p-1.5 rounded-md text-amber-400 hover:text-amber-300 shadow-lg">
          ✏️
        </button>
        <button onClick={() => onEliminar(producto.id)} className="bg-slate-900/90 backdrop-blur-md border border-slate-600 p-1.5 rounded-md text-red-400 hover:text-red-300 shadow-lg">
          🗑️
        </button>
      </div>
      ...
      <div className="flex flex-col grow gap-1 mb-2 md:mb-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-[10px] md:text-xs font-bold uppercase tracking-wide px-2 py-0.5 rounded-md whitespace-nowrap ${
              producto.stock > 0
      ...
      <div className="mt-auto pt-2 md:pt-3 border-t border-slate-700/50 grid grid-cols-[auto_1fr] gap-2 items-center w-full">
        <SelectorCantidad cantidad={cantidad} onDecrementar={decrementar} onIncrementar={incrementar} />
        ...
        <button
          ...
          className={`h-9 md:h-10 px-2 rounded-lg font-bold text-white text-[10px] md:text-xs uppercase tracking-wide shadow-lg transition-all transform active:scale-95 flex items-center justify-center gap-1 w-full
                        ${producto.stock === 0 ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'bg-linear-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500'}`}
        >
          Agregar
        </button>
      </div>
